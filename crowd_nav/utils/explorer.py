...
class Explorer(object):
    ...
    def run_k_episodes(self, k, phase, update_memory=False, imitation_learning=False, episode=None,
        ...
        for i in range(k):
            ...
            try:
                ob = self.env.reset(phase=phase, test_case=None)
            ...
            while not done:
                ...
                # Calculate path length based on action type
                if hasattr(action, 'vx') and hasattr(action, 'vy'):
                    # For actions with vx and vy (e.g., ActionLinear)
                    length += 0.25 * np.linalg.norm([action.vx, action.vy])
                elif hasattr(action, 'v'):
                    # For actions with v (e.g., ActionRot)
                    length += 0.25 * abs(action.v)
                else:
                    logging.warning(f"Unsupported action type: {type(action)}. Path length not updated.")
        ...
        assert success + collision + timeout == k, "Mismatch in episode counts"
        avg_nav_time = sum(success_times) / len(success_times) if success_times else self.env.time_limit
        avg_path_length = sum(self.robot_path_length_list) / len(self.robot_path_length_list) if self.robot_path_length_list else 0
